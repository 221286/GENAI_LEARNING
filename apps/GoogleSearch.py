...
"""
agent=create_react_agent(llm,tools,prompt=systemInstruction,checkpointer=memory)
while True :
   query=input("Users :")
   if(query.lower() in ["bye","bad","quit","exit"]):
      print("bye bye")
      break 
   response=agent.invoke({"messages":[("user", query)]},config={"recursion_limit": 10,"configurable": {
            "thread_id": "1"
        }},
                         )
